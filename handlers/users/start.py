...
async def bot_start(message: types.Message, state: FSMContext):
    if not db.user_exists(message.from_user.id):
        ...
        try:
            ...
            await bot.send_message(message.from_user.id,text,reply_markup=get_lang_for_button(message))
        except:
...
@dp.message_handler(text=["/start","/change_language","/ask","/about",'/registration'],state=RegistrationStates.name)
async def er(message: types.Message, state: FSMContext):
    ...
    if lang == 'uz':
        ...
        await bot.send_message(message.from_user.id, "Введите свое имя и фамилию")
